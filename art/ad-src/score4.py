"""THE STANDARD — warm, human, building, resolved-but-open. 44s. G major."""
...
for st,n in ((1.2,-14),(3.6,-10),(6.2,-7),(8.6,-14)):
    add(key(hz(n),4.2,0.26), st, 0.5)
add(strings(hz(-26),12.0,0.10), 2.0)

# 10–22s: a second voice answers. the room gets less lonely.
for st,n,p in ((10.4,-7,0.35),(12.0,-2,0.62),(13.8,-10,0.38),(15.4,-7,0.60),
               (17.0,1,0.42),(18.6,-2,0.58),(20.2,-7,0.45)):
    add(key(hz(n),3.6,0.24), st, p)
add(strings(hz(-26),13.0,0.13), 10.0); add(strings(hz(-19),13.0,0.10), 10.6)

# 22–32s: it builds — chords under the turn. G – D – Em – C
for st,ns in ((22.0,[-26,-14,-10,-7]),(24.6,[-19,-7,-2,1]),
              (27.2,[-24,-12,-5,0]),(29.8,[-21,-9,-2,5])):
    for k,n in enumerate(ns):
        add(strings(hz(n),3.4,0.13-0.012*k), st, 0.5)
        if k>1: add(key(hz(n),2.6,0.15), st+0.05*k, 0.4+0.1*k)
    add(pulse(0.14), st)

# 32–40s: the ask. full but warm.
for st,ns in ((32.4,[-26,-14,-7,-2,5]),(36.4,[-24,-12,-5,0,7])):
    for k,n in enumerate(ns):
        add(strings(hz(n),4.6,0.14-0.011*k), st, 0.5)
        add(key(hz(n),3.4,0.16-0.014*k), st+0.04*k, 0.42+0.05*k)
...
for n,a in ((-26,0.20),(-14,0.14),(-7,0.13),(1,0.11),(5,0.10)):   # G D B(add9→A) D
    add(strings(hz(n),5.0,a), 39.6, 0.5)
    add(key(hz(n),5.4,a*0.9), 39.8, 0.5)
...
print("wrote public/score-standard.wav 44s, G major")
